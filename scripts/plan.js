...
function getReadyReportList() {
        ...
        xmlhttp.open("GET", "MarketplaceWebService/Samples/GetReportListSample.php?requestID=" + requestReportID, true);
        xmlhttp.send();
...
function getReadyReport() {
        ...
        xmlhttp.open("GET", "MarketplaceWebService/Samples/GetReportSample.php?reportId=" + reportID, true);
        xmlhttp.send();
...
function getUsefulData(result){
    ...
    for(var i=0; i< lines.length; i++){
        ...
        plan.push({SKU:sku,ASIN:asin, SalesUnits:salesUnits,FBAInv: fbaInv, suggestUnits: suggestUnits});
...
function readyDataToSheet(){
	...
	if(readyToSheet.length <= 2){
		document.getElementById("txtHint").innerHTML = "!!Please enter at least one quantity!!";
	}
