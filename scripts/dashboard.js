...
window.onload = function() {
  ...
  caSalesDataRef.on('value', function(snapshot){
    ...
    for (var t = 0; t < Object.keys(typeSaleTable).length; t++){
      options +="<option value='"+Object.keys(typeSaleTable)[t]+"'>"+Object.keys(typeSaleTable)[t]+"</option>";
    }
  ...
  var myPieChart = new Chart(ctx2,{
    ...
    data: {
      datasets:[{
        data: pieData,
        backgroundColor: ["#8b008b","#1e90ff","#b22222","#fffaf0","#ffd700","#ff8c00","#8fbc8f"
...
function drawAllPnS(){
  ...
  for(var sku in PnSItemTable){
    $("#myCanvasDiv").append(
      "<div class='col'>"+
        "<canvas class='my-4 w-100' id='"+sku+"Canvas' width='900' height='380'></canvas>"+
      "</div>"
          ...
          if (range.values.length > 0) {
            ...
            for (var sku in PnSItemTable){
              ...
              for(var i = 0; i < PnSItemTable[sku].AveSales.length; i++ ){
                ...
                console.log("cost " + parseFloat(costList[costSKU].substring(1))*1.3);
                console.log("Ave Sales " + (Number(PnSItemTable[sku].AveSales[i])));
                profitList.push((parseFloat(PnSItemTable[sku].PriceList[i]) - parseFloat(costList[costSKU].substring(1))*1.3) * PnSItemTable[sku].AveSales[i] );
              }
              console.log("profitList " + profitList);
              drawPnSTable(sku+"Canvas",PnSItemTable[sku].PriceList, PnSItemTable[sku].AveSales, profitList);
            }
          } else {
